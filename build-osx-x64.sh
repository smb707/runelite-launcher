...
APPBASE="build/macos-x64/Areos.app"
...
build() {
    ...
    cp native/build-x64/src/Areos $APPBASE/Contents/MacOS/
...
dmg() {
    ...
    mv Areos\ *.dmg Areos-x64.dmg

    # dump for CI
    hdiutil imageinfo Areos-x64.dmg

    if ! hdiutil imageinfo Areos-x64.dmg | grep -q "Format: UDBZ" ; then
        echo "Format of resulting dmg was not UDBZ, make sure your create-dmg has support for --format"
        exit 1
    fi

    if ! hdiutil imageinfo Areos-x64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit Areos-x64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple Areos-x64.dmg
    fi
}
